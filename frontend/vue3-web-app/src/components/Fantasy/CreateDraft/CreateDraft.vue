      ...
      <div class="sticky-parent right-0 d-flex">
        <div class="sticky-child" style="min-width: 100px;">
          ...
          <span :class="{ 'sticky-child': true, 'd-flex': true, 'gold': true, 'invalid': totalGold > 600 }"
            ...
            <GoldSpan :font-size="1.0" :validation="totalGold > 600" :gold-value="(600 - totalGold).toFixed(0)" />
          </span>
        </div>
        ...
        <v-btn v-else class="btn-fantasy sticky-child" style="pointer-events: none;" disabled @click="saveDraft()">Save
          Draft</v-btn>
      </div>
    </div>
        ...
        <div class="sticky-child d-flex">
          <div class="sticky-child mt-3" style="min-width: 60px;">
            ...
            <span :class="{ 'sticky-child': true, 'd-flex': true, 'gold': true, 'invalid': totalGold > 600 }"
              ...
              <GoldSpan :font-size="0.8" :validation="totalGold > 600" :gold-value="totalGold.toFixed(0)" />
            </span>
            <span :class="{ 'sticky-child': true, 'd-flex': true, 'gold': true, 'invalid': totalGold > 600 }"
              style="top: 93px;font-size:0.8rem">Remaining:
              <GoldSpan :font-size="0.8" :validation="totalGold > 600" :gold-value="(600 - totalGold).toFixed(0)" />
            </span>
          </div>
          <v-btn class="btn-fantasy sticky-child" style="top:58px;font-size:0.7rem" @click="clearDraft()">Clear
            Draft</v-btn>
          <v-btn
            v-if="authenticated && leagueStore.selectedFantasyLeague && leagueStore.isDraftOpen(leagueStore.selectedFantasyLeague!) && totalGold <= 600"
            class="btn-fantasy sticky-child" style="top:58px;font-size:0.7rem" @click="saveDraft()">Save
            Draft</v-btn>
          <v-btn v-else class="btn-fantasy sticky-child" style="top:68px;font-size:0.7rem; pointer-events: none;"
            disabled @click="saveDraft()">Save
            Draft</v-btn>
        </div>
      </div>
    </div>
