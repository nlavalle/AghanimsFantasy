...
class DotaClient
{
    ...
    public DotaClient(ILogger<DotaClient> logger)
    {
        _logger = logger;

        userName = Environment.GetEnvironmentVariable("STEAM_CLIENT_USER") ?? "";
        password = Environment.GetEnvironmentVariable("STEAM_CLIENT_PASSWORD") ?? "";

        client = new SteamClient();
        clientReady = false;

        // get our handlers
        user = client.GetHandler<SteamUser>() ?? throw new Exception("Dota Client - No Steam User Set");
        gameCoordinator = client.GetHandler<SteamGameCoordinator>() ?? throw new Exception("Dota Client - Unable to get Steam Game Coordinator");
    ...
    public bool IsReady()
    {
        // I don't want to make this public so people can't set it
        return clientReady;
    }
    ...
    public void Wait()
    {
        ...
        while (listening && DateTime.Now.Ticks - start < TimeSpan.FromSeconds(10).Ticks)
        {
    ...
    public CMsgDOTAMatch? RequestMatchDetails(ulong matchId)
    {
        if (!clientReady)
        {
            _logger.LogError("Client is not ready to request match details");
            return null;
        }

        _logger.LogInformation("Requesting details of match {0}", matchId);
        ...
        // Clear any previous match settings
        Match = null;
    ...
    {
        _logger.LogInformation("Connected! Logging '{0}' into Steam...", userName);
    ...
    void OnLoggedOn(SteamUser.LoggedOnCallback callback)
    {
        if (callback.Result != EResult.OK)
        {
            ...
            _logger.LogError("Unable to logon to Steam: {0}", callback.Result);

            listening = false; // we didn't actually get the match details, but we need to jump out of the callback loop
            return;
        }

        _logger.LogInformation("Logged in! Launching DOTA...");
    ...
    void OnClientWelcome(IPacketGCMsg packetMsg)
    {
        ...
        _logger.LogInformation("GC is welcoming us. Version: {0}", msg.Body.version);
    ...
    void OnMatchDetails(IPacketGCMsg packetMsg)
    {
        ...
        {
            _logger.LogError("Unable to request match details: {0}", result);
        }
