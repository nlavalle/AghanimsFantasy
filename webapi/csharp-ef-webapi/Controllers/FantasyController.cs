...
namespace csharp_ef_webapi.Controllers
{
    ...
    public class FantasyController : ControllerBase
    {
        ...
        // GET: api/fantasy/players/5/matches/points
        [HttpGet("players/{leagueId}/matches/points")]
        public async Task<ActionResult<List<FantasyPlayerPoints>>> GetFantasyPlayersPointsByMatch(int? leagueId, [FromQuery] int? limit)
        {
            if (leagueId == null)
            {
                return BadRequest("Please provide a League ID to fetch fantasy player points of");
            ...
            var fantasyPlayerPointTotals = await _fantasyRepository.FantasyPlayerPointsByFantasyLeagueAsync(leagueId.Value, limit.Value);
            return Ok(fantasyPlayerPointTotals);
        }

        // GET: api/fantasy/players/5/matches/points
        [Authorize]
        [HttpGet("draft/{leagueId}/matches/points")]
        public async Task<ActionResult<List<FantasyPlayerPoints>>> GetDraftFantasyPlayersPointsByMatch(int? leagueId, [FromQuery] int? limit)
        {
            ...
            var fantasyPlayerPointTotals = await _fantasyRepository.FantasyPlayerPointsByFantasyDraftAsync(userDiscordAccountId, leagueId.Value, limit.Value);
            return Ok(fantasyPlayerPointTotals);
        }
        ...
        public async Task<ActionResult<List<MetadataSummary>>> GetFantasyLeagueMatchMetadata(int fantasyLeagueId)
        {
            ...
            matchSummary = matchSummary.OrderBy(m => m.FantasyPlayer.DotaAccount.Name).ToList();
